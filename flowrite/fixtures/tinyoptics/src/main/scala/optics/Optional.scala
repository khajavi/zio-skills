...
/**
 * An `Optional` focuses on a part `A` that *may be absent* inside `S`, yet can
 * be replaced when present. It is the meet of [[Lens]] (which can always `set`)
 * and [[Prism]] (whose focus may not match): it has `getOption` like a prism and
 * `set` like a lens.
 *
 * `Optional` is what you get when you compose a lens with a prism — for example,
 * "the radius of the shape stored in a person's favourite field", where the
 * field is always present but the shape might not be a circle:
 *
 * {{{
 ...
 * @param set       replace the focus, returning the whole unchanged if absent
 */
