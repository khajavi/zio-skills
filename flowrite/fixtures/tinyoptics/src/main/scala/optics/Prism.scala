...
/**
 * A `Prism` focuses on one case `A` of a sum type `S` that *may or may not*
 * match — typically one branch of a sealed trait, or a successful parse. It can
 * try to extract the focus (`getOption`) and can always rebuild the whole from a
 * focus (`reverseGet`).
 *
 ...
 * }}}
 *
 * @param getOption   try to extract the focus from the whole
 * @param reverseGet  rebuild the whole from a focus
 */
final case class Prism[S, A](getOption: S => Option[A], reverseGet: A => S) {
  ...
  def modify(f: A => A): S => S =
    s => getOption(s).fold(s)(a => reverseGet(f(a)))
  ...
  def andThen[B](that: Prism[A, B]): Prism[S, B] =
    Prism(
  ...
  def andThen[B](that: Optional[A, B]): Optional[S, B] =
    asOptional.andThen(that)

  /** Compose with a [[Lens]] focused inside the matched case. */
  def andThen[B](that: Lens[A, B]): Optional[S, B] =
    asOptional.andThen(that.asOptional)

  /**
   * Widen this `Prism` to an [[Optional]]. `set` replaces the focus only when
   * the whole already matches this prism's case.
   */
