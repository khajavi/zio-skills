...
val ageLens = new Lens[Person, Int](
  get = _.age,
  set = newAge => p => p.copy(age = newAge)
)

val alice = Person("Alice", 30)
val updated = ageLens.set(31)(alice)  // Person("Alice", 31)
```
...
val successPrism = new Prism[Result, String](
  getOption = {
...
val updated = successPrism.modify(_ + "!")(result)  // Success("OK!")

val failed: Result = Failure("Error")
val unchanged = successPrism.modify(_ + "!")(failed)  // Failure("Error") — unchanged
```
...
val composedOptional: Optional[Person, String] = 
  personResultLens.andThen(successPrism)

val composedPrism: Prism[Result, Int] = 
  resultPrism.andThen(valuePrism)
```
...
val cityLens = new Lens[Address, String](
  get = _.city,
...
val textPrism = new Prism[Message, String](
...
val personAddressLens = new Lens[Person, Address](...)
val addressCityLens = new Lens[Address, String](...)
...
// Result: Person("Alice", Address("Main St", "New York"))
```
...
The project includes 6 runnable example programs that demonstrate:

- `LensBasics` — Simple Lens creation and usage
- `LensOperations` — Get, set, and modify operations
- `LensComposition` — 2-level Lens composition
- `DeepComposition` — 3-level composition without `.copy()` pyramids
- `OptionalAndPrism` — Prisms and Lens-Prism composition
- `CompleteExample` — All concepts in a realistic scenario

Run with: `sbt "runMain examples.<Name>"`

---

## FAQ

**Q: When should I use a Lens vs a Prism?**

A: Use a Lens when the focused field is always present (product types like case classes). Use a Prism when the value might not match a pattern (sum types like sealed traits).

**Q: Can I compose Lenses of different types?**

A: Yes! A Lens composed with a Prism creates an Optional, allowing you to mix product and sum types in a composition chain.

**Q: What's the difference between `set` and `modify`?**
