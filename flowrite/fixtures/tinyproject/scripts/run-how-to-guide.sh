#!/usr/bin/env bash
#
# run-how-to-guide.sh — run write-how-to-guide against this fixture, then archive
...
#
# Name a TASK, not a subject. "Lens" is what run-data-type-ref.sh takes; this one wants something a
# reader could finish — "compose two optics over one field". A subject-shaped request is the case the
# gate is supposed to refuse, so it tests classification rather than writing.
#
# Because the INT trap archives, Ctrl-C after the gate's classification line appears is the cheap
# check on the riskiest part of this kind: `tutorial` and `how-to` share docs/guides/, nothing
# downstream notices a mix-up, and one turn is enough to see which the gate chose.
#
# Usage: bash scripts/run-how-to-guide.sh "<task>" [skip-phase1,skip-phase2,...]
#   Skip phases: write-examples, integrate, review — e.g.
#   bash scripts/run-how-to-guide.sh "compose two optics over one field" write-examples,integrate,review
set -uo pipefail
...
task="${1:?usage: run-how-to-guide.sh <task> [skip-phase1,skip-phase2,...]}"
skip_phases="${2:-}"
log="$(mktemp)"
echo "flue log: $log"
...
request="Please write a how-to guide for this task: $task."
...
cleanup() {
  ...
  bash scripts/archive-docs.sh "$log" write-how-to-guide
  rm -f "$log"
  exit 130
}
