...
#   module-name: tally (2 types, cheap) or optics (4 types, exercises scale). The fixture holds both,
#     so the run has to scope its research to the one named and leave the other alone.
...
#   bash scripts/run-module-ref.sh "tally" hierarchical research,design
#
# FLUE_VERBOSE_TOOLS defaults to 1 because full tool args and results are the only way to audit
# which phase actually wrote a page. Export 0 for a readable log when the phase timeline is all you
# need: FLUE_VERBOSE_TOOLS=0 bash scripts/run-module-ref.sh "tally" hierarchical
set -uo pipefail
