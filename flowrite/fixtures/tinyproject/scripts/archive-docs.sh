...
# patch under ../<fixture>-archive/<workflow>-turn<N>/changes.patch, AND
# writes two full, standalone, runnable project copies alongside it:
#   <fixture>-base/  — committed HEAD, unmodified (what the run started from)
#   <fixture>-final/ — HEAD + this run's changes merged (what the run produced)
# <fixture> is this directory's own name, derived below rather than written out, so copying this
# script to another fixture cannot leave it archiving into the wrong one.
...
#    resetting anything.
final_count="$(copy_tree "$dest/$fixture-final")"
...
# invented (an old module run left REVIEW_SUMMARY.txt, a module-review.md and a copied checklist
...
base_count="$(copy_tree "$dest/$fixture-base")"
...
echo "archived turn $n: $changed file(s) changed. $base_count file(s) in $dest/$fixture-base/, $final_count file(s) in $dest/$fixture-final/$log_note"
