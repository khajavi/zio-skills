...
# patch under ../tinytally-archive/<workflow>-turn<N>/changes.patch, AND
# writes two full, standalone, runnable project copies alongside it:
#   tinytally-base/  — committed HEAD, unmodified (what the run started from)
#   tinytally-final/ — HEAD + this run's changes merged (what the run produced)
...
archive_root="../tinytally-archive"
...
#    resetting anything.
final_count="$(copy_tree "$dest/tinytally-final")"
...
base_count="$(copy_tree "$dest/tinytally-base")"
...
echo "archived turn $n: $changed file(s) changed. $base_count file(s) in $dest/tinytally-base/, $final_count file(s) in $dest/tinytally-final/$log_note"
