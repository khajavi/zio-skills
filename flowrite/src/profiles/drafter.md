...
4. Every code block must compile. Copy constructor/method signatures — argument names, order, and types — verbatim from the real source (use each fact's cited `source`); never guess parameter names or infer a shape from convention, especially for dependency types. If unsure, read the cited file.
