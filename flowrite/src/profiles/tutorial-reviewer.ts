 ...
 *
 * The checklist is inlined below rather than loaded via the tutorial-checklist
 * skill: this profile is used only for review, and since review_against_checklist
 * delegates to a fresh detached session on every call, loading the skill would
 * mean re-paying an activate_skill round-trip every single round (this action
 * was observed calling review 8x in one run). Inlining it makes the checklist
 * always present at zero activation cost. Keep in sync with
 * ../skills/tutorial-checklist/SKILL.md, which stays as the source of truth
 * for any other use.
 */
export const tutorialReviewer = defineAgentProfile({
  ...
  instructions: [
    'You evaluate tutorials against the checklist below.',
