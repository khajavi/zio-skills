...
Fix only the listed violations — do not rewrite, reorder, or improve anything else. Preserve mdoc modifiers and code block semantics exactly: never split or merge code blocks, and never re-declare a `val` that an earlier block already defines — mdoc blocks share one scope, so a duplicate definition breaks compilation. Every task must reach `completed` before you finish.
