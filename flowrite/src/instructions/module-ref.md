...
3. **Design.** Call `design_module_plan` with the module name and the exact research object to
   ...
   and the type order. Motivation applies only when a history finding gives a REASON the module is
   factored this way. If the run requested a specific shape or layout, pass it as `shapeOverride` /
