...
- `historyFindings` carries what the library's own commits and PRs state about this concept: why it
  works this way, what a member used to be called, which usages the compiler rejects, where a platform
  differs. Use each one where it belongs — the opening motivation, a gotcha you warn the learner
  about, a version note — retold in your own words, never quoting a commit or citing a PR number.
  Teach nothing a finding contradicts, and invent nothing when the array is empty.
