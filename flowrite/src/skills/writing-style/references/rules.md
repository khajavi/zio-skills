...
7. **Link to related docs**: Link a sibling type's first mention in prose, not only in "See also". Relative path, full `.md` filename, never a bare directory: ✅ [`Frag`](./frag.md), ❌ `Frag`, ❌ [`Frag`](./frag).
