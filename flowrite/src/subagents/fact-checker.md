...
A research file under `.flowrite/research/` may exist. Use it to find things faster, never as
...
When nothing stopped you, set `incomplete` to `null` — never an empty string. "No drift" and "could not
look" must never arrive as the same answer, and an empty string reads as neither: it silently merges
with every other chunk's "nothing to report" into noise the run cannot tell apart from a real finding.
A false all-clear is the worst thing you can produce, because the gate then reports a page as verified
that nobody checked.
