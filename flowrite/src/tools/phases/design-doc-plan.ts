    ...
    return {
      output: await designPlan({
        ...
        guidance: [
          ...
          `The research's "historyFindings" decides "motivation": set it true when one of them gives a`,
          `real reason the type exists, false otherwise — a non-empty array is not enough on its own,`,
          `since a finding can be a rename or a platform note that motivates nothing. A Motivation`,
          `section with nothing behind it is a section the drafter has to invent.`,
        ],
  ...
  async run({ harness, data, log }) {
    ...
    const plan = await designPlan({
      ...
      guidance: [
        ...
        ``,
        `Decide which module-level sections apply — "motivation" follows the research's "historyFindings":`,
        `true when one of them gives a real reason the module is factored this way, false otherwise (a`,
        `rename or a platform note motivates nothing). Organize EVERY type into named groups in reading order.`,
