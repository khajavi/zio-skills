    ...
    return {
      output: await writeDoc({
        ...
        prompt: [
          ...
          `designRationale carries WHY the API is shaped this way, read from the repo's own commit and`,
          `PR history — draw the Motivation / Use Case section and per-method caveats from it. If it is`,
          `empty, write no motivation rather than inventing one.`,
    ...
    return {
      output: await writeDoc({
        ...
        prompt: [
          ...
          `designRationale carries WHY the module is factored this way, read from its commit and PR`,
          `history — draw the motivation and any design notes from it, and invent none if it is empty):`,
    ...
    return {
      output: await writeDoc({
        ...
        prompt: [
          ...
          `verbatim code/signatures to copy exactly. designRationale carries WHY the`,
          `concept works this way, read from the repo's commit and PR history — use it`,
          `for the motivation and the gotchas, and invent neither if it is empty):`,
          JSON.stringify(data.researchAnswers),
          ``,
