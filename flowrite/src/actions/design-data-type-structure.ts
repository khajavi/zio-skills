  ...
  coreOperationCategories: v.pipe(
    v.array(
      v.object({
        ...
        methods: v.pipe(
          v.array(v.string()),
          v.description('Method names in this category. Avoid a category with a single method — merge singletons into a related category, since a lone subsection violates the no-lone-subheader rule.'),
        ),
      }),
    ),
  ...
  async run({ harness, input, log }) {
    ...
    const { data } = await session.task(
      [
        ...
        `public operation into ordered Core Operations categories (no single-method`,
        `category — merge singletons). Ground every choice in these research answers:`,
