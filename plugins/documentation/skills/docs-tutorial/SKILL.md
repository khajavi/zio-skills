...
Fix every reported drift by correcting the **tutorial**, never the source. Bounded rounds, same
discipline: fix everything a check reports, then confirm once — that confirming round is what records
the tutorial as clean. Genuinely new drifts in a confirming round earn another, up to 3 total; a round
...
content and have it evaluate the tutorial against every item.

```
Task(
  description: "Review <tutorial-id> tutorial",
  subagent_type: "documentation:docs-reviewer",
  prompt: "Evaluate docs/guides/<tutorial-id>.md against this checklist: <CHECKLIST.md's content>"
)
```
