...
Write the complete documentation file(s) and save them to the specified output path(s).`;
    ...
    const reviewPromptResult = await session.prompt(
      `**Phase 4: Review and fix module-ref**\n\nCall the \`review_docs\` action to run the critic/fix loop on the module-ref you just wrote. Report its result as a fenced json block titled REVIEW_RESULT, e.g.:\nREVIEW_RESULT\n\`\`\`json\n{ "approved": true, "rounds": 1, "findingsFixed": {"HIGH":0,"MEDIUM":0,"LOW":0}, "unresolvedIssues": [] }\n\`\`\``
    );
